...
static void itemLoader(byte num)
{
	...
	setMenuOption_P(5, PSTR(STR_HAERTS), menu_heart, mHeartsOpen);
	
	setMenuOption_P(6, PSTR(STR_PRESSURES), menu_pressure, mPressuresOpen);
	
	setMenuOption_P(7, PSTR(STR_GYRS), menu_gyrocope, mGyoscopeOpen);
	
	setMenuOption_P(8, PSTR(STR_LETTERS), menu_letter, mLetterOpen);
	
	setMenuOption_P(9, PSTR(STR_DIAGNOSTICS), menu_diagnostic, mDiagOpen);
